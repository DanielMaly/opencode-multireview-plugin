...
Immediately spawn all specialists in the final effective roster concurrently after routing and applicable preflight succeed. Supply every specialist with the same complete high-level scope string, including the target changeset and any caller-supplied plan, ticket, specification, recorded decision, or phase/PR-slice identifier. Do not introduce a separate scope transport. Do not add detailed instructions unless the caller requests them; each specialist owns its peer-domain review.
